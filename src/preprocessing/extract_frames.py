...
ADAPTIVE PIPELINE:
    ...
    - No hard crash on low frame count â warning only
    - Motion detection via optical flow
    - Quality scoring system
    - max_frames=300  (T&T benchmark needs 150â300 images; was 150 for small-object scans)
...
def run_smart_frame_selection(image_dir: str, budget: int = 300) -> dict:
    ...
    Falls back to uniform subsampling when core.frame_selection is unavailable.
    """
    image_dir_path = Path(image_dir)
    report_path = _frame_selection_report_path(image_dir_path)

    try:
        from core.frame_selection import SmartFrameSelectionEngine  # type: ignore
    ...
    # --- Fallback: uniform subsample to budget ---
    frames = _image_files(image_dir_path)
    original_count = len(frames)

    if original_count > budget:
        import math
        step = original_count / budget
        keep_indices = {int(i * step) for i in range(budget)}
        removed = 0
        for i, p in enumerate(frames):
            if i not in keep_indices:
                try:
                    ...
                    pass
        selected_count = original_count - removed
    else:
    ...
    print(
        f"[selection] Fallback uniform selection: "
        f"{selected_count}/{original_count} frames kept (budget={budget})"
    )
    return report
...
def validate_image_resolution(image_dir: str, min_size: int = 256) -> None:
    ...
    ~256px â it produces near-zero features and a degenerate sparse model
    ...
    if bad_images:
        ...
        raise RuntimeError(
            ...
            f"Tiny images produce near-zero SIFT features â blank splat.\n\n"
            f"Offending images:\n{errors}\n\n"
            f"Fix: re-upload original full-resolution photos or video."
        )

    print(f"[validate_resolution] â All images meet minimum {min_size}x{min_size}px requirement.")
...
def _find_binary(name: str) -> str:
    ...
    raise RuntimeError(
        ...
        "  Windows: https://ffmpeg.org/download.html  â  extract to C:\\ffmpeg\\"
    )
...
def estimate_motion(image_dir: str) -> float:
    ...
    try:
        ...
        print("[extract] â  opencv not installed â skipping motion estimation.")
        return 0.1
    ...
    # ð¥ normalize low motion (prevents 0.00 issue)
    if motion < 0.01:
        motion = 0.1
...
def filter_blurry_images(image_dir: str, threshold: float = 120.0) -> int:
    ...
    threshold=120 is moderate â rejects clearly blurry frames while keeping enough for COLMAP.
    ...
    try:
        ...
        print("[extract] â   opencv-python not installed â skipping blur filter.")
    ...
    if kept < 15:
        print("[extract] â   WARNING: Very few sharp frames! Try recording in better light.")
    return kept
...
def validate_exposure(image_dir: str, overexposed_thresh: float = 245.0,
    ...
    Frames are not removed â this is a diagnostic/advisory function only.

    Args:
        image_dir:           Directory of extracted frames.
        overexposed_thresh:  Mean pixel value above which a frame is flagged (0â255).
    ...
    """
    try:
        from core.dataset_analysis.exposure_analyzer import analyze_exposure  # type: ignore
    ...
    # --- Fallback: local cv2 implementation ---
    try:
        import cv2
    except ImportError:
    ...
    if bad_total / max(total, 1) > bad_ratio_limit:
        ...
        if underexposed > 0:
            parts.append(f"{underexposed} underexposed (too dark)")
        warning = (
            f"[extract] â   Exposure warning: {', '.join(parts)} out of {total} frames. "
            "Lock camera exposure before recording to avoid COLMAP failures."
        )
        print(warning)
    else:
...
def filter_low_feature_frames(
    ...
    try:
        ...
        print("[extract] â  opencv-python not installed â skipping feature filter.")
        return len(_image_files(Path(image_dir)))
    ...
    for p in frames:
        ...
        if img is not None:
            # ð¥ CLAHE BOOST (huge improvement)
...
def filter_duplicate_viewpoints(
    image_dir: str,
    ssim_threshold: float = 0.98,  # raised 0.88â0.98: only remove truly identical viewpoints
    hist_threshold: float = 0.995, # raised 0.96â0.995: keep frames with even slight viewpoint change
    min_keep_ratio: float = 0.85,  # raised 0.5â0.85: guarantee at least 85% of frames survive
) -> int:
    """
    ...
    Two-stage filter:
      1. Histogram correlation â O(N) pass: any frame with hist_corr >= hist_threshold
         compared to its nearest retained neighbour is dropped.
      2. SSIM pass on close pairs only â catches frames that differ in brightness
    ...
    try:
        ...
        print("[diversity] â  opencv not installed â skipping viewpoint diversity filter.")
        return len(_image_files(Path(image_dir)))
    ...
    print(f"[diversity] Scanning {len(frames)} frames for near-duplicate viewpoints â¦")
    ...
    for i in range(1, len(frames)):
        ...
        # else: near-duplicate of previous retained frame â candidate for removal
    ...
    for idx in kept_indices[1:]:
        ...
        mse = diff.mean()
        # approx SSIM: high MSE â different viewpoint â keep
        ssim_approx = 1.0 - (mse / (255.0 ** 2))
        if ssim_approx < ssim_threshold:
            final_kept.append(idx)
        # else: SSIM too similar â skip (duplicate)

    # Enforce min_keep_ratio
    min_keep = max(1, int(len(frames) * min_keep_ratio))
    if len(final_kept) < min_keep:
        # Not enough diversity detected â fall back to uniform subsample
        step = len(frames) / min_keep
        final_kept = [int(i * step) for i in range(min_keep)]
        print(f"[diversity] â  Too few diverse frames â falling back to uniform subsample ({min_keep} frames)")
...
def _densify_frames_optical_flow(output_dir: Path, target_frames: int = 80) -> int:
    ...
    warping.  Each synthetic frame is a valid intermediate camera pose â
    ...
    Strategy: one round of interpolation at Î±=0.5 (midpoint).  If after one
    round we still have fewer than target_frames, do a second round at Î±=0.5
    ...
    try:
        ...
        print("[densify] â  opencv not installed â skipping frame densification.")
    ...
    if current >= target_frames:
        print(f"[densify] Already have {current} frames (â¥{target_frames}) â skipping densification.")
        return current

    print(
        f"[densify] â¡ Short-video densification: {current} â target {target_frames} frames "
    ...
    for _round in range(3):
        ...
        for i in range(len(frame_list) - 1):
            ...
            # Compute dense optical flow AâB
            flow = cv2.calcOpticalFlowFarneback(
                gray_a, gray_b, None,
            ...
            # Name: insert between f_a and f_b â use stem + "_mid" suffix
        ...
        print(
            f"[densify] Round {_round+1}: synthesised {new_count} midpoint frames â "
            f"{final_count} total"
        )
    ...
    print(f"[densify] â  Densification complete: {final} frames in {output_dir}")
    return final
...
def _renumber_frames(output_dir: Path) -> None:
    ...
    frames = _image_files(output_dir)
    # Two-pass rename to avoid collisions: temp names â final names
    import tempfile, os
    tmp_names = []
...
def extract_from_video(
    ...
    output_dir:        str,
    fps:               float = None,
    max_frames:        int   = 300,   # raised 150â300: 3-min video needs dense coverage
    blur_threshold:    float = 50.0,  # lowered 80â50: outdoor phone footage has natural blur
    ...
    SHORT_VIDEO_THRESHOLD = 25    # seconds â triggers high-density extraction
    SHORT_VIDEO_MIN_FRAMES = 150  # raised 100â150: more frames from short videos
    ...
    try:
        ...
        if fps is None:
            if is_short_video:
                # Short video: extract aggressively â cap at native fps to avoid
                ...
                fps = round(fps, 1)
                print(
                    f"[extract] â¡ SHORT VIDEO ({duration:.1f}s) â boosting to "
                    f"{fps:.1f} fps (target â¥{SHORT_VIDEO_MIN_FRAMES} frames, "
                    f"native={native_fps:.1f} capped at {effective_native:.0f})"
                )
            elif duration < 40:
                # Medium duration 25â40 s: 4 fps gives ~100â160 frames.
                fps = 4
            elif duration < 120:
                # Standard object-capture clip 40â120 s: 3 fps gives ~120â360 frames.
                fps = 3
            elif duration < 300:
                # Long scene video 2â5 min: 2 fps â ~240â600 frames (capped by max_frames).
                # Raised from 1â2: 3-min video now yields ~360 raw frames before filtering.
                fps = 2
            else:
        ...
        print(
            f"[extract] {info['width']}Ã{info['height']} @ {native_fps:.1f} fps | "
    ...
    # Keep original frame resolution (1280Ã830 in the required pipeline).
    ...
    print(f"[extract] FFmpeg saved {saved} frames â {output_dir}")
    ...
    # the effective frame count up WITHOUT re-recording â each synthetic frame
    ...
    if saved < 25:
        ...
        print(f"[extract] â  Too few frames â retrying at fps={fps_retry}")
    ...
    # Force GC before file rename operations â Windows holds file locks until
    ...
    # ------------------ FEATURE FILTER (ð¥ FIXED) ------------------
    kept = filter_low_feature_frames(
        str(output_dir),
        min_features=50,      # ð¥ reduced from 500
        min_keep_ratio=0.9    # raised 0.7â0.9: keep even more frames for dense coverage
    )

    # ð¥ SAFETY: never allow 0 frames
    if kept == 0:
    ...
    if motion < 1.0:
        print("[extract] â  LOW MOTION â move camera around object")

    # ------------------ FINAL SAFETY ------------------
    # Hard fail: COLMAP needs >= 30 frames for reliable reconstruction.
    # Lowered 40â30: the COLMAP quality gate catches bad reconstructions; this
...
def validate_images(image_dir: str) -> None:
    try:
        ...
        print("[extract] Pillow not installed â skipping validation.")
        return
    ...
    for p in sorted(image_dir.glob("*")):
        ...
        try:
            with Image.open(p) as img:
                ...
                # Hard minimum raised to 256px â anything smaller is useless
            ...
            print(f"[extract] â Removing bad image: {p.name} ({e})")
            bad.append(p)
...
def copy_images(
    ...
    for i, src in enumerate(images):
        ...
        if i % 50 == 0:
            print(f"[extract] Copying {i + 1}/{len(images)}â¦")
...
if __name__ == "__main__":
    ...
    parser.add_argument("input",
                        help="Video file (.mp4 / .mov / â¦) or image directory")
