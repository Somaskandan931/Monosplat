...
- [Tanks and Temples Benchmark](#tanks-and-temples-benchmark)
  - [What Tanks and Temples Is](#what-tanks-and-temples-is)
  - [Why T&T Footage Is the Right Fit for MonoSplat](#why-tt-footage-is-the-right-fit-for-monosplat)
  - [The Evaluation Metric: F-score](#the-evaluation-metric-f-score)
  - [Pipeline Configuration for T&T Footage](#pipeline-configuration-for-tt-footage)
  - [Running the F-score Evaluator](#running-the-f-score-evaluator)
  - [Where to Get the T&T Dataset](#where-to-get-the-tt-dataset)
...
MonoSplat's pipeline is designed and validated against footage and standards consistent with the [Tanks and Temples benchmark](https://www.tanksandtemples.org/) — the most widely-used evaluation standard for real-world 3D scene reconstruction.

### What Tanks and Temples Is

Tanks and Temples is a public benchmark released alongside the paper *Tanks and Temples: Benchmarking Large-Scale Scene Reconstruction* (Knapitsch et al., SIGGRAPH Asia 2017). It provides a curated dataset of high-quality real-world scenes captured with synchronized video cameras and measured against precise laser-scan ground-truth geometry. It has become the standard reference for evaluating photogrammetry and neural rendering pipelines in research.

The benchmark contains two scene sets:

| Set | Scenes | Character |
|-----|--------|-----------|
| **Training** | Barn, Caterpillar, Church, Courthouse, Ignatius, Meetingroom, Truck | Medium-scale objects and structures, controlled capture |
| **Advanced** | Auditorium, Ballroom, Courtroom, Museum, Palace, Temple | Large-scale indoor and outdoor architecture, complex lighting |

Each scene was captured by a camera slowly orbiting or walking around a static subject — exactly the capture pattern that COLMAP and Gaussian Splatting are optimised for.

### Why T&T Footage Is the Right Fit for MonoSplat

The benchmark's capture conditions match MonoSplat's core design assumptions almost exactly:

- **Static subject, moving camera** — COLMAP's Structure-from-Motion is built for this. A moving subject invalidates SfM; a moving camera is what it solves.
- **High frame overlap** — Consecutive frames share 60–80% of their content, giving COLMAP dense correspondences for accurate pose estimation.
- **Realistic lighting** — Outdoor and indoor lighting with real shadows, specularity, and varying albedo — conditions where Gaussian Splatting's view-dependent SH colour representation provides genuine value over simpler rendering methods.
- **One continuous capture** — No cuts, fades, or transitions. COLMAP requires a continuous keyframe graph; a single arc around the subject produces exactly this.

This is why T&T-style footage produces the best results from MonoSplat's pipeline. A shaky or jump-cut video with the same subject will reconstruct far less successfully.

### The Evaluation Metric: F-score

The standard training quality metrics used during training — PSNR, SSIM, LPIPS — measure **rendering quality**: how closely the model reproduces held-out photographs from known viewpoints. They say nothing about whether the underlying 3D geometry is accurate.

The Tanks and Temples benchmark uses a different metric: **F-score at threshold τ**.

```
Precision = fraction of predicted points within τ metres of any laser-scan point
Recall    = fraction of laser-scan points within τ metres of any predicted point
F-score   = 2 × Precision × Recall / (Precision + Recall)
```

This directly measures geometric reconstruction accuracy against a physical ground truth — whether the recovered 3D structure matches the real-world object, not just whether it looks good from the training viewpoints.

Threshold τ is scene-dependent. The benchmark recommends:

| Scene set | τ (threshold) |
|-----------|--------------|
| Training scenes (Truck, Barn, etc.) | 0.05 m (5 cm) |
| Advanced scenes (Temple, Palace, etc.) | 0.10 m (10 cm) |

A high PSNR and a low F-score together indicate the model has learned to reproduce training-view appearances well but has filled occluded regions with geometrically incorrect Gaussians. A balanced PSNR and F-score indicates both photometric and geometric accuracy.

### Pipeline Configuration for T&T Footage

MonoSplat ships with configuration values tuned for T&T-scale footage. The key differences from small-object defaults are:

| Parameter | Small object default | T&T tuned | Why |
|-----------|---------------------|-----------|-----|
| `max_gaussians` | 80,000 | **200,000** | Buildings and outdoor structures require far more Gaussians to cover surface area without gaps |
| `iterations` | 12,000 | **20,000** | More training views and larger scene extent need more convergence steps |
| `densify_until_iter` | 8,000 | **16,000** | Densification must run deep into training for large scenes — stopping early leaves regions unresolved |
| `densification_interval` | 500 | **200** | More frequent densification fills coverage gaps across a large area faster |
| `densify_grad_threshold` | 0.0005 | **0.0003** | Slightly more aggressive split/clone for scenes where each view covers only a small fraction of the total surface |
| `data.max_frames` | 180 | **300** | T&T benchmark submissions use 150–300 registered images minimum |
| `lambda_lpips` | 0.05 | **0.1** | Higher perceptual weight matters more for large outdoor structures where texture sharpness is visible across the scene |
| `colmap.camera_model` | OPENCV | **SIMPLE_RADIAL** | Official T&T dataset images were captured on professional cameras with negligible radial distortion. Use OPENCV for phone-recorded footage of a T&T-style subject |

> **Note on camera model:** `SIMPLE_RADIAL` is correct when using the official T&T image downloads. If you are shooting your own T&T-style video with a phone, keep `OPENCV` — phone lenses have real radial distortion that `OPENCV` models correctly.

### Running the F-score Evaluator

MonoSplat includes `scripts/evaluate_tanksandtemples.py`, a self-contained F-score evaluator that computes the benchmark metric against any laser-scan ground-truth PLY.
...
python scripts/evaluate_tanksandtemples.py \
    --pred  outputs/exports/final.ply \
    --gt    /data/tanksandtemples/Truck/Truck.ply \
    --scene Truck \
    --out   outputs/tt_eval/
```

**All training scenes at once:**
```bash
python scripts/evaluate_tanksandtemples.py \
...
**Output per scene:**
- Console table: Precision / Recall / F-score at τ
- `<scene>_tt_eval.json`: machine-readable metrics
- `<scene>_colored.ply`: predicted cloud coloured green (true positive, within τ) and red (false positive, outside τ) — lets you see exactly where the reconstruction is geometrically accurate and where it drifts

The coloured PLY is the most useful debugging output: regions that are red but should be occupied indicate coverage gaps caused by insufficient training views or too-low `max_gaussians`. Regions that are green but float above the surface indicate Gaussians that have drifted due to insufficient pruning.

### Where to Get the T&T Dataset

The official image sets and laser-scan ground truth are available for download at:

**[https://www.tanksandtemples.org/download/](https://www.tanksandtemples.org/download/)**

The training set (7 scenes) is freely available. Ground-truth PLYs are provided for F-score evaluation. The advanced set requires registration.
